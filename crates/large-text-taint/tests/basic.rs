...
";
    ...
    // mov x4, x3 — non-start row is INPUT-ONLY: snapshot is exactly {x3}.
    // x4 (dst) is the already-resolved taint we came from; the backward
    // walk now aims at x3, so that's the only reg to highlight.
    let mov_x4 = by_line
        .values()
        .find(|r| {
            let tl = &p.lines()[r.index];
    ...
    assert!(
        mov_x4.reg_snapshot[x3],
        "mov x4 row must contain x3 (new upstream target)"
    );
    assert!(
        !mov_x4.reg_snapshot[x4],
        "mov x4 row must NOT contain x4 (dst, already resolved)"
    );
    assert!(!mov_x4.reg_snapshot[x0], "mov x4 row must NOT carry x0 downstream");

    // mov x3, x10 — input-only: snapshot is exactly {x10}.
    let mov_x3 = by_line
        .values()
        .find(|r| {
            let tl = &p.lines()[r.index];
    ...
    assert!(
        ...
        "mov x3 row must contain x10 (new upstream target)"
    );
    assert!(
        !mov_x3.reg_snapshot[x3],
        "mov x3 row must NOT contain x3 (dst, already resolved)"
    );
    assert!(
